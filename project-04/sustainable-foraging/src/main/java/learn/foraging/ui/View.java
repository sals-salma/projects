...
public class View {
    ...
    public void displayKgPerItemReport(Map<String, Double> report) {
        if(report == null || report.isEmpty()){
            io.println("No report found.");
            return;
        }
    ...
    public void displayCategoryValue(Map<Category, BigDecimal> report) {
        ...
        for(Category category : report.keySet()){
            io.printf("%s : $%s \n", category, report.get(category).setScale(2, RoundingMode.HALF_UP));
        }
    }
}
