...
public class Controller {
    ...
    private void reportCategoryValue() {
        ...
        Map<Category, BigDecimal> report = forageService.reportTotalPerCategory(date);
        view.displayCategoryValue(report);
        view.enterToContinue();
    }

    private void reportKgPerItem() {
        //1. Ask the user for date
        view.displayHeader(MainMenuOption.REPORT_KG_PER_ITEM.getMessage());
        LocalDate date = view.getForageDate();
        Map<String, Double> report = forageService.reportKgPerItems(date);
        view.displayKgPerItemReport(report);
        view.enterToContinue();
    }
    ...
    private void viewForagers() {
        ...
        List<Forager> foragers = foragerService.findByState(String.valueOf(state));
